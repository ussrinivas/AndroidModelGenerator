...
class SlickGenerator(database: Database) {
  ...
  private def convertToProjection(m: Models): String = {
    m.fieldsList.map { f =>
      CodeGenUtils.convertToLowerCamelCase(f.name)
    } mkString ("", " ~ ", "")
  }

  private def convertTableFields(m: Models): String = {
    m.fieldsList.map { f =>
      "  def " +CodeGenUtils.convertToLowerCamelCase(f.name) + " = column[" + f.fieldType + "](\"" + f.name + "\")"
    } mkString ("", "\n", "")
  }
  
  private def getModel(m: Models): ActionResult = {
    var content = CodeGenUtils.readTemplate(Play.classloader.getResourceAsStream("templates/slick.template"))

    content = content.replace(CodeGenUtils.packageTemplate, database.packageName)
    content = content.replace(CodeGenUtils.modelTemplate, m.name)
    content = content.replace(CodeGenUtils.classTemplate, CodeGenUtils.convertToUpperCamelCase(m.name))
    content = content.replace("${tableFields}", convertTableFields(m))
    content = content.replace("${projection}", convertToProjection(m))    
